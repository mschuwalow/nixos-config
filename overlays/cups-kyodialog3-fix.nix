self: super:
let
  platform = if super.stdenv.hostPlatform.system == "x86_64-linux" then
    "64bit"
  else if super.stdenv.hostPlatform.system == "i686-linux" then
    "32bit"
  else
    throw "Unsupported system: ${super.stdenv.hostPlatform.system}";
  region = "Global";
...
in {
  cups-kyodialog3 = super.cups-kyodialog3.overrideAttrs (oldAttrs: rec {
    src = super.fetchzip {
      url =
        "https://www.kyoceradocumentsolutions.us/content/download-center-americas/us/drivers/drivers/Kyocera_Linux_PPD_Ver_${
          builtins.replaceStrings [ "." ] [ "_" ] oldAttrs.version
        }_tar_gz.download.gz#Kyocera_Linux_PPD_Ver_${oldAttrs.version}.tar.gz";
